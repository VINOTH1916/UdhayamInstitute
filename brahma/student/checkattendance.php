...
// Get the student ID based on roll number
$student_query = "SELECT student_id FROM students WHERE roll_no = '$roll_no'";
...
// Determine the appropriate attendance table based on roll number
$attendance_table = "attend_";
...
for ($day = 1; $day <= $daysInMonth; $day++) {
    ...
    $attendanceQuery = "SELECT status FROM $attendance_table WHERE student_id = '$student_id' AND date = '$date'";
    ...
    if ($attendanceResult->num_rows > 0) {
        ...
        // Default status if no data found for the day
        $status = "not-taken";
    }
    ...
    $calendarDays[] = ['day' => $day, 'class' => $class];
}

// Get total and present days for the attendance percentage calculation
$attendance_query = "SELECT COUNT(*) as total, SUM(CASE WHEN status = 'present' THEN 1 ELSE 0 END) as present FROM $attendance_table WHERE student_id = '$student_id'";
...
<body>
    <div class="container">
        ...
        <div class="calendar">
            ...
            // Pad the calendar with empty days for the first week
            for ($i = 0; $i < $firstDayOfMonth; $i++) {
                echo '<div class="day normal-day"></div>';
            }

            // Get attendance data for the current month
            foreach ($calendarDays as $calendarDay) {
                echo '<div class="day ' . $calendarDay['class'] . '">' . $calendarDay['day'] . '</div>';
            }
            ?>
        </div>
        <br>
